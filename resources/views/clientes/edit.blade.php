...
<body class="bg-gray-900 text-white min-h-screen flex flex-col items-center justify-center">
  ...
  <form action="{{ route('clientes.update', $cliente) }}" method="POST"
    ...
    <div class="mb-4">
      ...
      <input type="text" name="nombre" value="{{ $cliente->nombre }}" autocomplete="off"
             class="w-full px-3 py-2 rounded-lg bg-white/20 border border-white/30 text-white focus:outline-none focus:ring-2 focus:ring-yellow-500">
    </div>

    <!-- Correo -->
    <div class="mb-4">
      <label class="block text-white/90 text-sm font-bold mb-2">Correo electrónico</label>
      <input type="email" name="correo" value="{{ $cliente->correo }}" autocomplete="off"
             class="w-full px-3 py-2 rounded-lg bg-white/20 border border-white/30 text-white focus:outline-none focus:ring-2 focus:ring-yellow-500">
    </div>
    ...
    <div class="flex justify-center">
      <button type="submit"
              class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-3 px-8 rounded-xl shadow-md hover:shadow-yellow-400/40 transition-all duration-200">
        💾 Guardar
      </button>
    </div>
  </form>
